import type { Inquiry } from "./_schema";
...
export async function sendInquiryEmail(
  ...
  const toEmail = env.INQUIRY_NOTIFICATION_EMAIL;

  if (!apiKey || !toEmail) {
    console.warn("Email skipped: RESEND_API_KEY or INQUIRY_NOTIFICATION_EMAIL not configured");
    return;
  }

  const photoBase = r2PublicUrl ?? "";
  const photoSection =
    photoKeys.length > 0
      ? `\n\nPHOTOS (${photoKeys.length} submitted)\n${photoKeys
          .map((key, i) => `  ${i + 1}. ${photoBase ? `${photoBase}/${key}` : key}`)
          .join("\n")}`
      : "\n\nPHOTOS\n  None submitted";

  const submitted = inquiry.createdAt
    ? new Date(inquiry.createdAt).toLocaleString("en-US", { timeZone: "America/New_York" })
    : inquiry.createdAt;
  ...
  <div style="background:#f9f9f9;padding:24px;border-radius:0 0 8px 8px;border:1px solid #e0e0e0">
    ...
    ${
      photoKeys.length > 0
        ...
        ${photoKeys.map((key) => `<li><a href="${photoBase}/${key}" style="color:#1a7a1a">${key}</a></li>`).join("")}
      </ul>`
    ...
    <div style="background:#1a2e1a;color:#fff;padding:12px 16px;border-radius:6px;margin-top:16px;font-size:13px">
      Inquiry #: ${inquiry.inquiryNumber} &nbsp;|&nbsp; Source: ${inquiry.source ?? "Website"} &nbsp;|&nbsp; ${submitted}
    </div>
  </div>
</div>
`;

  // Normalise RESEND_FROM_EMAIL — accept "email@domain" or "Name <email@domain>"
  const rawFrom = env.RESEND_FROM_EMAIL ?? "";
  const emailMatch = rawFrom.match(/<(.+)>/);
  const fromAddress = emailMatch ? emailMatch[1]!.trim() : rawFrom.trim();
  const from = fromAddress
    ? `Rehman INC <${fromAddress}>`
  ...
  console.log(`Inquiry email sent: ${inquiry.inquiryNumber}`);
}
