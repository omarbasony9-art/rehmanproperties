...
import { ArrowRight, CheckCircle, Home, Calendar, Shield } from "lucide-react";
...
export default function HomePage() {
  ...
  useSEO(
    content.seoTitle || "Sell Your House for Cash | Rehman INC",
    content.seoDescription || "Skip the repairs and stress. Rehman INC buys houses directly from homeowners for cash in any condition. Get a straightforward, no-obligation cash offer today.",
  ...
  // Hero image — use admin-set URL if available, else default asset
  const heroImage = content.heroImage?.trim()
    ? content.heroImage
    : `${import.meta.env.BASE_URL}hero-home.jpg`;

  const ctaText = content.heroCta || cfg.main_cta_text || "GET MY CASH OFFER";
    ...
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/20">
        ...
        <div className="container mx-auto px-4 max-w-5xl">
          <div className="text-center mb-16">
            ...
            {content.whyUsSubtitle && (
              <p className="text-muted-foreground text-lg text-center mt-4 max-w-2xl mx-auto">{content.whyUsSubtitle}</p>
            )}
          </div>
          ...
          <div className="text-center">
            <Button size="lg" asChild className="h-14 px-10 text-lg">
              <Link href="/sell-your-house">
                Sell Your House <ArrowRight className="ml-2 w-5 h-5" />
              </Link>
            </Button>
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
          ...
          <div className="w-full max-w-2xl mx-auto bg-background p-2 md:p-3 rounded-lg shadow-2xl flex flex-col md:flex-row gap-2">
            ...
            <Button
              ...
              className="h-14 md:h-16 px-8 text-lg font-bold w-full md:w-auto shrink-0 shadow-md"
            >
              {content.finalCtaButton || ctaText} <ArrowRight className="ml-2 w-5 h-5" />
            </Button>
          </div>
        </div>
      </section>
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full p-4 bg-background/90 backdrop-blur-md border-t border-border z-40">
        <Button size="lg" className="w-full h-14 text-lg font-bold shadow-xl" onClick={() => setFormOpen(true)}>
          {cfg.main_cta_text || "Get My Cash Offer"}
        </Button>
      </div>
